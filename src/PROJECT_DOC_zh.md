...
flowchart TB
  ...
  subgraph LLM["大模型任务层 ylhb_llm + ylhb_interfaces"]
    ...
    VIN["voice_input_node<br/>CZ007 录音 + ASR"]:::ai
...
└── ylhb_llm/
    ...
    ├── voice_input_node.py              # CZ007/ALSA录音、ASR、发布文字指令
...
| `voice_input_node` | `ylhb_llm` | CZ007/ALSA 麦克风录音 | `/retail_ai/text_command` | 把语音指令转成文字指令；默认关闭，接上声卡后再开启 |
...
| `audio_device` | `default` | ALSA 录音设备 | CZ007 若是 `hw:2,0`，启动时传 `audio_device:=hw:2,0` |
| `record_sec` | `4.0` | 每次录音秒数 | 短命令 3~5 秒，太长会增加延迟 |
| `sample_rate` | `16000` | 录音采样率 | 中文短语音 16000 足够 |
| `enabled` | `false` | 是否启用语音输入 | 没接 CZ007 时保持 false |
...
| `audio_device` | `default` | ALSA 播放设备 | 根据 `aplay -l` 调整 |
...
# 接入 CZ007，声卡是 hw:2,0，只开语音输入，不播放
ros2 launch ylhb_llm llm.launch.py \
  enable_voice:=true \
  enable_tts:=false \
  audio_device:=hw:2,0
...
#### 6.7.9 CZ007 语音输入和 TTS 播放

当前机器如果还没插入 CZ007，`arecord -l` 可能显示没有声卡。插上 CZ007 后先确认：
...
如果看到 USB 麦克风或声卡设备，再启动语音输入。先只开 ASR，不开 TTS：
...
如果 `arecord -l` 显示的设备不是 default，例如 `hw:2,0`，启动时指定：
...
```text
CZ007 麦克风 -> voice_input_node -> 云端 ASR -> /retail_ai/text_command -> retail_task_node
```
...
```text
retail_task_node -> /retail_ai/say_text -> voice_output_node -> 云端 TTS -> aplay 播放
```
...
如果使用语音，arecord -l 和 aplay -l 能看到 CZ007 或可用声卡
```
